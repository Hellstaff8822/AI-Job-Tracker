...
export async function deleteJobAction(id: string) {
  try {
    ...
    throw new Error(`Не вдалося видалити: ${error.message}`);
  }
}
  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'Ти — професійний IT-рекрутер. Твоє завдання: проаналізувати вакансію і надати короткий план підготовки: 5 питань для інтерв’ю, Elevator Pitch та ключові поради. Відповідь давай українською мовою у форматі Markdown.'
        },
        {
          role: 'user',
